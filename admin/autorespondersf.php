...
class ZPage extends XPage
{
    ...
    function ocd_list ()
    {
        ...
        $this->pageTitle = "Inactive Members Autoresponder Email Templates";
        $this->pageHeader = "Inactive Members Autoresponder Email Templates";

        $total = $this->db->GetOne ("Select Count(*) From {$this->object} Where `is_free`=1");
        $this->data = array (
            "MAIN_HEADER" => $this->pageHeader,
            "MAIN_ADDLINK" => "<a href='{$this->pageUrl}?ocd=new' title='To add a template'><img src='./images/add.png' border='0'></a>",
            "HEAD_SUBJECT" => "Subject",
            "HEAD_DAYS" => "Days After Sign Up",
        ...
        if ($total > 0)
        {
            $result = $this->db->ExecuteSql ("Select * From `{$this->object}` Where `is_free`=1 Order By z_day Asc");
            while ($row = $this->db->FetchInArray ($result))
            {
                ...
                $activeLink = "<a href='javascript:is_active(\"".$this->object."\", \"email_id\", ".$email_id.")'><img src='./images/active".$row['is_active'].".png' width='25' border='0' alt='Change activity status' title='Change activity status' /></a>";
    ...
    {
        $this->mainTemplate = "./templates/autoresponderf_details.tpl";
        $id = $this->GetGP ("id");
    ...
    function ocd_new ()
    {
        ...
        $this->pageHeader = "<a href='{$this->pageUrl}'>Inactive Members Autoresponder Email Templates</a> / New Template";
    ...
    function ocd_insert ()
    {
        ...
        {
            $this->db->ExecuteSql ("Insert into {$this->object} (subject, message, z_day, is_active, is_free) values ('$subject', '$message', '$z_day', '0', '1')");
    ...
    function ocd_edit ()
    {
        ...
        $this->pageHeader = "<a href='{$this->pageUrl}'>Inactive Members Autoresponder Email Templates</a> / Edit Template";
