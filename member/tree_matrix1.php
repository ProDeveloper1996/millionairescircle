...
class ZPage extends XPage
{
    ...
    {
        GLOBAL $dict;
        $this->mainTemplate = "./templates/tree_matrix1.tpl";

        if ($this->LicenseAccess['tree_matrix'][$this->lic_key]==0) exit('access denied');

        $id = $this->GetGP ("id", $this->member_id);

        $matrix_type = $this->db->GetSetting ("cycling", 0);

        if ($matrix_type == 1) // cycling
            $this->pageHeader = '<a href="./matrix.php"></a>'. '  ' . $dict['MX_Tree'] ;
        else // forced
            $this->pageHeader = $dict['Menu_MyMatrix'] ;

        $this->pageTitle = $dict['Menu_MyMatrix'];
        ...
        if ($matrix_type == 0) // forced
        {
            ...
            if ( $this->db->GetOne("Select count(matrix_id) from matrix where member_id=$id ")==0)
                $content='Please make <a href="payment_f.php">payment</a> to enter this structure';
...
/*  TREE cycling */
    function matrix_tree_cycling($host_id=1, $m_level=1){
        ...
        while ($row = $this->db->FetchInArray($result))
        {
            ...
            $tmp = str_replace('{title}', "Circle #".$i, $tmp);
            $tmp = str_replace("{mH}", (!empty($host_id)?'<p>'.$host_id.'</p>':""), $tmp);

            $this->cycling_getMembersCircle($host_id, $place_id, $width, $depth, 0);
//            d($this->dataCircle);
            for($m=0;$m<=13;$m++) {
                $tmp = str_replace("{m$m}", (!empty($this->dataCircle[$m])?$this->dataCircle[$m]:""), $tmp);
            }
            $content.= $tmp;
        }
    ...
    function cycling_getMembersCircle($host_id, $place_id, $width, $depth, $sts,  $reentry=1, $arrow=false){
        $sts++;
        ...
        if ($count_ref > 0)
        {
            ...
            while ($row = $this->db->FetchInArray ($result))
            {
                ...
                $this->dataCircle[] = '<p>'.$member_id.$member_info.'</p>';

                $i++;
                if ($sts < $depth)
                {
                    $this->cycling_getMembersCircle ($host_id, $new_place_id, $width, $depth, $sts,  $reentry, true);
                }
            }
        }
        if ($count_ref < $width)
        {
            ...
            {
                $this->dataCircle[] = '<img src=\'./images/grey_man.png\' border=\'0\' alt=\'Vacant Place\' title=\'Vacant Place\' />';
                if ($sts < $depth)
                {
                    $this->cycling_getMembersCircle ($host_id, -1, $width, $depth, $sts);
                }
            }
        }
    }
...
function getMemberInfo ($member_id, $reentry, $host_id, $new_place_id='')
{
        ...
        if ($is_dead == 0)
        {
                ...
                $popover = "$name<br>$add<br>Level:$mm_level";
                ...
                $toRet .= '<span class="fa-stack fa-1" data-container="body" data-toggle="popover" data-trigger="hover" data-html=true data-placement="left" data-content="'.$popover.'"><i class="fa fa-info"></i></span>';
...
/*  END TREE FORCED */
    ...
    {
        return '
            <div style="width: 750px;height: 750px;zoom:0.7;">
                <p style="position: absolute;width:750px;text-align: center;font-size: 26px;font-family:OpenSans-Bold;color: #6d6d6d;">{title}</p>
                <div class="canvas" style="margin: 260px 0 0 264px;position: absolute;background: #a5a5a5">
                    <span class="cn" style="left: 84px;">{mH}</span>
                    <svg class="chart" width="200" height="250" viewBox="0 0 50 50"></svg>
                </div>
        
                <div class="canvas" style="margin: 8px 0 0 0px;position: absolute;">
                    <span class="cn" style="right: 240px;left: initial;top: 117px;">{m9}</span>
                    <span class="cn" style="right: 90px;left: initial;top: 270px;">{m10}</span>
                    <span class="cn" style="right: 90px;left: initial;bottom: 270px;">{m12}</span>
                    <span class="cn" style="right: 236px;left: initial;bottom: 112px;">{m13}</span>
                    <span class="cn" style="left: 236px;right: initial;bottom: 112px;">{m6}</span>
                    <span class="cn" style="left: 90px;right: initial;bottom: 270px;">{m5}</span>
                    <span class="cn" style="left: 90px;right: initial;top: 270px;">{m3}</span>
                    <span class="cn" style="left: 240px;right: initial;top: 117px;">{m2}</span>
                    <svg class="chart" width="750" height="750" viewBox="0 0 50 50">
                        <circle class="unit r45" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r45" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r45" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r45" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r45" r="15.9" cx="50%" cy="50%"></circle>
                ...
                <div class="canvas" style="margin: 108px 0 0 100px;position: absolute;">
                    <span class="cn" style="right: 108px;left: initial;top: 135px;">{m8}</span>
                    <span class="cn" style="right: 108px;left: initial;bottom: 135px;">{m11}</span>
                    <span class="cn" style="left: 108px;right: initial;bottom: 135px;">{m4}</span>
                    <span class="cn" style="left: 108px;right: initial;top: 135px;">{m1}</span>
                    <svg class="chart" width="550" height="550" viewBox="0 0 50 50">
                        <circle class="unit r90" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r90" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r90" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r90" r="15.9" cx="50%" cy="50%"></circle>
                    </svg>
                ...
                <div class="canvas" style="margin: 210px 0 0 200px;position: absolute;transform: rotate(90deg);">
                    <span class="cn" style="transform: rotate(-90deg);left: 150px;bottom: 50px;">{m0}</span>
                    <span class="cn" style="transform: rotate(-90deg);left: 150px;top: 50px;">{m7}</span>
                    <svg class="chart" width="350" height="350" viewBox="0 0 50 50">
                        <circle class="unit r180" r="15.9" cx="50%" cy="50%"></circle>
                        <circle class="unit r180" r="15.9" cx="50%" cy="50%"></circle>
                    </svg>
                </div>
